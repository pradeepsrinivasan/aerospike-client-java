...
public final class BatchNode {
        ...
        if ( policy.replica == Replica.CUSTOM) {
            ...
            for( Entry<Node, HashSet<Key>> e :prepareBatchNodes.entrySet()) {
				...
				for (Key key : e.getValue()) {
					...
					int findIndex = findIndex(key, keys);
					if (batchNode == null) {
						batchNodes.add(new BatchNode(node, keysPerNode, findIndex));
					}
					else {
						batchNode.addKey(findIndex);
					}
				}
	...
	private static int findIndex(Key key, Key[] keys) {
	...
	public static List<BatchNode> generateList(Cluster cluster,
			BatchPolicy policy, List<BatchRead> records)
			throws AerospikeException {
		Node[] nodes = cluster.getNodes();

		if (nodes.length == 0) {
			throw new AerospikeException(ResultCode.SERVER_NOT_AVAILABLE,
	...
	public void splitByNamespace(Key[] keys) {
		...
		if (isSingleNamespace(keys, first)) {
			...
			batchNamespaces
					.add(new BatchNamespace(first, offsets, offsetsSize));
			return;
		}
		...
		for (int i = 0; i < offsetsSize; i++) {
			...
			if (batchNamespace == null) {
				batchNamespaces
						.add(new BatchNamespace(ns, offsetsSize, offset));
			} else {
